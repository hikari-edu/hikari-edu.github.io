#!/usr/bin/env node
/*
 * Records the three public Hikari demonstrations against an isolated stack.
...
const supportedDemos = new Set(["competidor", "siem", "pesquisa"]);
if (!supportedDemos.has(demo)) {
  throw new Error("Usage: record_landing_demos.cjs <competidor|siem|pesquisa>");
}
...
async function recordSiem(page, player) {
  ...
  await page.getByRole("button", { name: /Acessar HIKARI/i }).click();
